function t(t, e, a) {
  ...
  (this.center = () => new o(this.x + 15, this.y + 15)),
    (this.destroy = () => {
      (this.active = 0),
        (s = 0),
        (i = +new Date()),
        particles.takePower(new o(this.x, this.y)),
        sfx.takePower();
    }),
    (this.n = () => {
      !this.active && +new Date() - i >= 4e3 && (this.active = 1),
        this.active && (s += 0.03) > 1 && (s = 1);
    }),
    (this.r = () => {
      this.active &&
        (c.save(),
...
function a(t, e, a, i, s, n) {
  ...
  const r = [
    ...
    l = [
      [
        [
          19, 0, 28, 11, 27, 21, 13, 17, 0, 28, 12, 26, 20, 34, 10, 39, 7, 56,
          16, 45, 24, 46, 22, 56, 36, 68, 32, 56, 39, 48, 48, 58, 65, 56, 53,
          50, 49, 40, 63, 43, 76, 30, 62, 33, 52, 27, 64, 16, 54, 0, 54, 12, 41,
          19, 33, 4,
        ],
        "black",
        "black",
  ...
  m(),
    ...
    (this.r = () => {
      c.save(),
        ...
        c.restore(),
        c.save(),
        c.translate(p.x + 18, p.y + 18),
        draw.r(h, [40, 40]),
        c.restore(),
        c.save(),
        c.translate(p.x + n.x + 18, p.y + n.y + 18),
        draw.r(h, [40, 40]),
        c.restore();
    });
}
function i(t, e, a, i, s, n) {
  ...
  (this.startFalling = () => {
    this.falling.active ||
      (sfx.fallingBlock(),
      (this.falling.active = 1),
      (this.falling.start = +new Date()));
  }),
    (this.getVelocity = () => new o()),
    (this.n = () => {
      ...
      if (this.falling.active && !this.falling.falling) {
        ...
        (this.falling.falling = 1),
          (this.active = 0),
          (this.falling.position = new o(this.x, this.y));
      } else if (this.falling.falling && !this.falling.dead) {
        const t = this.falling.velocity.get().normalize().mult(-0.017);
        t.add(gc.gravity.get().mult(0.3)),
          this.falling.velocity.add(t),
          ...
          this.falling.opacity < 0 &&
            ...
            (this.falling.opacity = 0),
            setTimeout(() => {
              (this.falling.active = 0),
                (this.falling.falling = 0),
                (this.falling.velocity = new o()),
                (this.falling.dead = 0),
                (this.active = 1),
                (this.x = e),
                (this.y = a);
      ...
      this.falling.active ||
        ...
        this.falling.opacity > 1 && (this.falling.opacity = 1));
    }),
    (this.r = () => {
      this.isMovable &&
        (c.save(),
        c.translate(h.x + i / 2, h.y + s / 2),
        draw.r(l, [40, 40]),
        c.restore(),
        c.save(),
        c.translate(h.x + n.x + i / 2, h.y + n.y + s / 2),
        draw.r(l, [40, 40]),
        c.restore(),
        c.save(),
        (c.strokeStyle = color.mechanics),
        c.moveTo(h.x + i / 2, h.y + s / 2),
        c.lineTo(h.x + n.x + i / 2, h.y + n.y + s / 2),
        c.stroke(),
        c.restore()),
        c.save(),
        ...
        c.scale(1, -1);
      for (let t = 0; t < Math.floor(this.w / 40); t++)
        c.save(),
          this.falling.active
            ? c.translate(40 * t + rInt(-1, 1), rInt(-1, 1))
            : c.translate(40 * t, 0),
          draw.r(r, [40, 38]),
          c.restore();
  ...
  (this.n = () => {
    ...
    (this.r = () => {
      if (
        (this.isMovable &&
          (c.save(),
          c.translate(d.x + i / 2, d.y + s / 2),
          draw.r(h, [40, 40]),
          c.restore(),
          c.save(),
          c.translate(d.x + n.x + i / 2, d.y + n.y + s / 2),
          draw.r(h, [40, 40]),
          c.restore(),
          c.save(),
          (c.strokeStyle = color.mechanics),
          c.moveTo(d.x + i / 2, d.y + s / 2),
          c.lineTo(d.x + n.x + i / 2, d.y + n.y + s / 2),
          c.stroke(),
          c.restore()),
        c.save(),
        c.translate(this.x, this.y),
        ...
        for (let t = 0; t < Math.floor(this.w / 40); t++)
          c.translate(40, 0), draw.r(l, [40, 8]);
        ...
        for (let t = 0; t < Math.floor(this.h / 40); t++)
          c.translate(40, 0), draw.r(l, [40, 8]);
        c.restore(),
          c.save(),
          c.rotate(-Math.PI / 2),
...
function n(t, e, a, i, s, n) {
  ...
  const d = +new Date();
  (this.n = () => {
    h.add(l.get().normalize().mult(0.001)),
      h.add(gc.gravity.get().mult(t)),
      l.add(h),
      r.add(l),
      h.mult(0),
      +new Date() - d >= s && (this.isActive = 0);
  }),
    (this.r = () => {
      const t = 1 - (+new Date() - d) / s;
      c.save(),
        c.translate(r.x + 20, r.y),
      ...
      return (
        ...
        (r =
          i && w
            ...
            : n[l].map((e, a) =>
                e.map((e, i) =>
                  i ? e : e.map((e, c) => e + ((n[t][a][i][c] - e) * h) / s),
                ),
              ))
      );
    }),
...
function o(t, e) {
  (this.x = t || 0),
    ...
    (this.angle = (t) =>
      t ? Math.atan2(this.y, this.x) : Math.atan2(t.y - this.y, t.x - this.x)),
    ...
    (this.get = () => new o(this.x, this.y)),
    (this.mag = () => Math.hypot(this.x, this.y)),
    (this.mult = (t) => ((this.x *= t), (this.y *= t), this)),
    (this.normalize = () => (this.mag() > 0 && this.div(this.mag()), this)),
    (this.perpendicular = () => {
      let t = this.x;
    ...
    (this.normal = (t) =>
      new o(this.x - t.x, this.y - t.y).perpendicular().normalize()),
    (this.center = (t) =>
      new o(this.x + (t.x - this.x) / 2, this.y + (t.y - this.y) / 2));
}
(() => {
  function t() {
  ...
  function a() {
    (gc.last = +new Date()),
      (function () {
        if (gc.paused) return;
        scene.n(),
          character.isGoingBack()
            ? ((gc.paused = 1), s(-1))
            : character.levelIsCompleted()
            ? ((gc.paused = 1), s(1))
            : character.isDead() && ((gc.paused = 1), i());
      })(),
      c.save(),
  ...
  (window.rInt = (t, e) => Math.floor(t + Math.random() * (e - t))),
    ...
    (window.onload = function () {
      ...
      (e = document.getElementById("app")),
        ...
        (gc.gravity = new o(0, -0.8)),
        t(),
        control.i(),
        scene.i(),
        a(),
        gc.canvas.addEventListener("click", (t) => {
          gc.splashScreen && (gc.splashScreen = 0),
    ...
    return {
      i: () => {
        window.addEventListener("keydown", (e) => {
          ("KeyA" !== e.code && "ArrowLeft" !== e.code) || (t[0] = 1),
            ("KeyW" !== e.code && "Space" !== e.code && "ArrowUp" !== e.key) ||
              (t[1] = 1),
            ("KeyD" !== e.code && "ArrowRight" !== e.code) || (t[2] = 1),
            "Digit1" === e.code && gc.changeQuality(1),
            "Digit2" === e.code && gc.changeQuality(0.75),
            "Digit3" === e.code && gc.changeQuality(0.5),
            "Digit4" === e.code && gc.changeQuality(0.1),
            "KeyM" === e.code && (gc.muted = !gc.muted);
        }),
          window.addEventListener("keyup", (e) => {
            ("KeyA" !== e.code && "ArrowLeft" !== e.code) || (t[0] = 0),
              ("KeyW" !== e.code &&
                "Space" !== e.code &&
                "ArrowUp" !== e.code) ||
                (t[1] = 0),
              ("KeyD" !== e.code && "ArrowRight" !== e.code) || (t[2] = 0);
          });
      },
      pressed: t,
    };
        ...
        "walk" === t
          ...
          n !== t &&
            ("jump" === t
              ...
              ? sfx.fall()
              : "die" === t && sfx.die(),
            o && !c ? (i = t) : ((s = new r(...a[t])), (n = t), (o = e)));
      },
      r: (e, h) => {
        let d = h || 1;
        c.translate(e.x + t[0] / 2, e.y + t[1] / 2),
          c.scale(l ? -d : d, -d),
          draw.r(s.n(), t),
          o &&
            s.isFinished() &&
            i &&
            ((s = new r(...a[i])), (n = i), (i = null), (o = 0));
      },
    };
  })()),
  (window.character = (() => {
    ...
    function f(t) {
      s.dying ||
        (t
          ? particles.dying(e.get().add(new o(0, h.y)), [
              color.dying1,
              color.dying2,
              color.dying3,
              color.dying4,
    ...
    return {
      ...
      reset: () => {
        (l = new o()),
          (e = map.getCharacterStart().get()),
          (p = t),
          characterAnimations.mirror(0 !== e.x),
          (s = { dying: 0, isDead: 0 }),
      ...
      n: () => {
        if (s.dying) {
          ...
          const t = l.get().normalize().mult(-0.017);
          return t.add(gc.gravity.get().mult(0.45)), l.add(t), e.add(l), 0;
        }
        const a = l.get().normalize().mult(-0.017);
        a.add(gc.gravity.get().mult(0.9)),
          control.pressed[0]
            ? (a.add(new o(-1, 0)), characterAnimations.mirror(1))
            : control.pressed[2] &&
              (a.add(new o(1, 0)), characterAnimations.mirror(0)),
          l.add(a),
          (l.x = Math.abs(l.x) < 4 ? l.x : (Math.abs(l.x) / l.x) * 4),
          e.add(l);
        const i = (function (e) {
          const a = { touches: [], sides: [], isOverFan: 0 };
          return (
            map.getMap().enemy.forEach((i) => {
              if (8 === i.type) {
                if (
                  e.x + h.x / 2 > i.x &&
                  e.x + h.x / 2 < i.x + 120 &&
                ...
                3 === i.type
                  ? i.active &&
                    i.center().distance(e.get().add(new o(h.x / 2, h.y / 2))) <
                      i.collisionRadius + 20 &&
                    ((d.done = 0), (p = t), i.destroy())
                  : i.center().distance(e.get().add(new o(h.x / 2, h.y / 2))) <
                      i.collisionRadius + 20 && f();
            }),
        ...
        i.touches.forEach((a) => {
          ...
          0 === a.side &&
            ...
            control.pressed[0] ||
              control.pressed[2] ||
              (2 === a.type ? (l.x /= 1.02) : (l.x /= 2)),
            Math.abs(l.x) > 0.1 && particles.addRunning(e, l)),
            1 === a.side &&
              ((e.x = a.intersect),
              control.pressed[0] &&
                l.y < 0 &&
                ...
                particles.addWall(e, -1),
                (p -= 0.07),
                control.pressed[1]
                  ? d.first &&
                    (l.add(new o(20, 15)),
                    characterAnimations.to("jump", 0, 1),
                    (d.first = 0),
                    (p -= 2.5))
                  : (d.first = 1))),
            3 === a.side &&
              ...
              control.pressed[2] &&
                ...
                control.pressed[1]
                  ? d.first &&
                    ...
                    (p -= 2.5))
                  : (d.first = 1))),
            2 === a.side && ((e.y = a.intersect), (l.y = l.y >= 0 ? 0 : l.y));
        }),
          -1 !== i.sides.indexOf(0) && l.y <= 0
            ? (control.pressed[0] || control.pressed[2]
                ? characterAnimations.to("walk")
                : y || characterAnimations.to("stay"),
              control.pressed[1] &&
                d.first &&
                (l.add(new o(0, 15)),
                characterAnimations.to("jump", 0, 1),
              ...
              d.first ||
                control.pressed[1] ||
                ((d.first = 1), (d.second = 0), (d.done = 0)),
              g &&
                (characterAnimations.to("drop", 1),
                particles.addJump(e, l.x),
                (g = 0)))
            : (g = 1),
          (!i.sides.length || p < 0) &&
            l.y < 0 &&
            (i.isOverFan || characterAnimations.to("fall"),
            control.pressed[1] &&
              (d.second || d.first) &&
              (l.apply(new o(0, 15)),
              characterAnimations.to("jump", 0, 1),
              ...
              (d.second = 0),
              (d.done = 1)),
            control.pressed[1] || d.first || d.done || (d.second = 1)),
          !control.pressed[1] && l.y > 0 && (l.y /= 1.2),
          e.x < 0 && map.isFirst() ? (e.x = 0) : e.x + h.x <= 0 && (r = 1),
          e.y + h.y < 0 && f(1),
          e.x >= map.getEnd().x + 40 && (n = 1),
          (control.pressed[0] || control.pressed[1] || control.pressed[2]) &&
            (w = +new Date()),
          +new Date() - w > 2e4
            ? y ||
              ((y = 1), characterAnimations.to(["dancing", "sit"][rInt(0, 2)]))
            : (y = 0);
      },
      nFinal: () => {
        if (a) (i -= 0.004) < 0 && (i = 0);
          ...
          t.add(new o(0.1, 0)),
            l.add(t),
            (l.x = Math.abs(l.x) < 1 ? l.x : (Math.abs(l.x) / l.x) * 1),
            e.add(l),
            e.x >= 1e3 - h.x / 2 &&
              ((e.x = 1e3 - h.x / 2),
              (a = 1),
      ...
      r: () => {
        c.save(),
          characterAnimations.r(e),
          c.restore(),
          p < t &&
            (c.save(),
            (c.fillStyle = color.stamina),
            c.fillRect(e.x - 10, e.y + h.y + 10, 4 * p < 0 ? 0 : 6 * p, 8),
            c.restore());
      },
      rFinal: () => {
        c.save(),
    ...
    const t = [
      [
        [
          ...
          425, 139, 385, 52, 373, 146, 352, 157, 350, 37, 330, 130, 310, 159,
          290, 151, 281, 38, 273, 163, 262, 88, 255, 141, 232, 39, 215, 141,
          203, 162, 180, 94, 174, 163, 152, 157, 143, 89, 117, 124, 113, 172,
          77, 149, 74, 85, 57, 164, 54, 82, 32, 173, 19, 60, 2, 160,
        ],
        "",
        "black",
        1,
  ...
  (window.map = (() => {
    ...
    let l = 0,
      h = { map: [], enemy: [], start: new o(), end: new o() };
    function d() {
      (h = { map: [], enemy: [], start: new o(), end: new o() }),
        r[n].forEach((n) => {
          4 === n[0]
            ? h.map.push(
                new i(
                  ...
                  n[3] * c,
                  n[4] * c,
                  (void 0 !== n[5] ? new o(n[5], n[6]) : new o()).get().mult(c),
                ),
              )
            : 5 === n[0]
            ? h.enemy.push(
            ...
            : 6 === n[0]
            ? (h.start = new o(n[1] * c, n[2] * c))
            : 7 === n[0]
            ? (h.end = new o(n[1] * c, n[2] * c))
            : 3 === n[0]
            ? h.enemy.push(new t(n[0], n[1] * c, n[2] * c))
            : 8 === n[0]
            ? h.enemy.push(new e(n[0], n[1] * c, n[2] * c))
            : h.map.push(
                new s(
                  n[0],
                  n[1] * c,
  ...
  (window.camera = (() => {
    ...
    return {
      reset: () => {
        ...
        t.apply(new o(e.x - gc.res.x / 2, e.y - gc.res.y / 2)),
          t.x < 0 && (t.x = 0),
          t.x + gc.res.x > map.getEnd().x + 40 &&
            (t.x = map.getEnd().x + 40 - gc.res.x),
          t.y < 0 && (t.y = 0);
      },
      n: () => {
        const a = character.position();
        e.apply(new o(a.x - gc.res.x / 2, a.y - gc.res.y / 2)),
          e.x < 0 && (e.x = 0),
          e.x + gc.res.x > map.getEnd().x + 40 &&
            (e.x = map.getEnd().x + 40 - gc.res.x),
          e.y < 0 && (e.y = 0),
          t.add(e.get().sub(t).mult(0.05));
      },
      r: () => {
        c.translate(-t.x, -t.y);
      },
    ...
    return {
      ...
      n: () => {
        if (i && +new Date() - a > 2500) {
          (l += 0.01) > Math.PI / 4 && (l = Math.PI / 4), (h -= 0.01);
          const t = n.get().normalize().mult(-0.017);
          t.add(new o(-0.2, 0.1)), n.add(t), s.add(n);
        }
      },
      r: () => {
        c.save(),
          ...
          c.scale(h, -h),
          c.rotate(l),
          draw.r(i ? e.n() : t, [97, 72]),
          c.restore();
      },
      rBackground: () => {
        let t = c.createLinearGradient(0, 0, 0, gc.res.y);
        t.addColorStop(0, "hsl(238, 10%, 30%)"),
          t.addColorStop(1, "hsl(238, 10%, 10%)"),
          c.save(),
          (c.fillStyle = t),
  ...
  (window.scene = (() => {
    ...
    return {
      i: () => {
        (t = c.createLinearGradient(0, 0, 0, gc.res.y)).addColorStop(
          0,
          "hsl(37, 30%, 45%)",
        ),
          t.addColorStop(1, "hsl(37, 30%, 10%)"),
          background.i(),
          map.i(),
          character.i();
      },
      ...
      n: () => {
        gc.splashScreen
          ...
          : (background.n(),
            map.n(),
            map.isLast() ? (character.nFinal(), finalScene.n()) : character.n(),
            particles.n(),
            camera.n());
      },
      r: () => {
        c.save(),
          ...
          gc.splashScreen
            ? splashScreen.r()
            : (map.isLast() ? finalScene.rBackground() : background.r(),
              c.save(),
              camera.r(),
              map.r(),
              map.isLast()
                ? (character.rFinal(), finalScene.r())
                : character.r(),
              particles.r(),
              c.restore()),
          c.save(),
  ...
  (window.sfx = (() => {
    ...
    function e(t, e, a) {
      ...
      (i.type = "triangle"),
        i.connect(s),
        s.connect(gc.ac.destination),
        (i.frequency.value = t),
        i.start(0),
        (s.gain.value = a || 1),
        s.gain.exponentialRampToValueAtTime(
          1e-5,
          gc.ac.currentTime + (e || 0.5),
        );
    }
    return {
      fall: () => {
      ...
      run: () => {
        +new Date() - t < 200 || (e(146.83, 0.05, 0.4), (t = +new Date()));
      },
      wall: () => {
        +new Date() - t < 100 || (e(41.2, 0.2), (t = +new Date()));
      },
      die: () => {
        e(61.74, 3);
      },
      ...
      flying: () => {
        +new Date() - t < 30 || (e(27.5, 0.5), (t = +new Date()));
      },
    };
  })()),
  (window.splashScreen = (() => ({
        ...
        for (let e = 0; e < 5; e++)
          t.push(
            new n(
              ...
              1,
              a.get(),
              new o(rFloat(-1, 1), rFloat(0.5, 0.5 + i.x / 5)),
              500,
              color.walking,
            ),
          );
        ...
        for (let a = 0; a < 5; a++)
          t.push(
            new n(
              rFloat(0.1, 0.15),
              1,
              e.get().add(new o(20 * i, 0)),
              new o(rFloat(-0.5, 0.5), rFloat(0.1, 0.5)),
              500,
              color.walking,
            ),
          );
        ...
        for (let i = 0; i < 50; i++)
          t.push(
            new n(
              ...
              1,
              e.get(),
              new o(rFloat(a - 2, a + 2), rFloat(0.5, 1)),
              500,
              color.walking,
            ),
          );
        ...
        for (let a = 0; a < 1; a++)
          t.push(
            new n(
              rFloat(1e-4, 15e-5),
              2,
              e.get().add(new o(rInt(0, 100), rInt(0, 20))),
              new o(0, rFloat(1, 3)),
              2e3,
              color.walking,
            ),
          ...
          t.push(
            new n(
              ...
              new o(
                ...
                rFloat(3, 4) * Math.cos(rFloat(0, 2 * Math.PI)),
              ),
              500,
              a[rInt(0, a.length)],
            ),
  ...
  (window.draw = (() => {
    ...
    return {
      r: (e, a, i) => {
        c.save(),
          a && c.translate(-a[0] / 2, -a[1] / 2),
          e.forEach((e) => {
            bp(),
              (c.fillStyle = color[e[2]] || e[2] || t),
              (c.strokeStyle = color[e[1]] || e[1] || t),
              (c.lineWidth = e[3] ? 0.001 : i),
              (c.lineJoin = "round"),
              m(e[0][0], e[0][1]);
